...
def extract_and_enhance_messages(raw: str, jd: str | None = None) -> list[dict]:
    ...
    return [
        {"role": "system", "content": (
            ...
            "'Oversaw' is fine; KEEP any metric already present in that bullet's "
            "original text, but NEVER invent, estimate or add a number, percentage, "
            "dollar amount or date that is not already there, and NEVER use a "
            "placeholder like 'X%', 'N%' or '$X' — if there is no real number, write "
            "the bullet cleanly without one; (c) ALWAYS include a "
            "'skills' section with 8-12 relevant skills drawn from the candidate's "
            "titles, summary and experience. Each skill MUST be a SHORT keyword of "
            "1-3 words (e.g. 'Python', 'Sales Strategy', 'Team Leadership', 'CRM', "
            "'Business Development') — NEVER a full sentence, an achievement, or an "
            "award title, and never put a metric inside a skill. Group them under "
            "simple category labels (e.g. 'Technical', 'Sales', 'Leadership'). "
            "Keep all companies, titles, "
            "dates and facts exactly; never invent employers, numbers or jobs.\n"
            "OUTPUT FORMAT (strict): every summary and highlight must be CLEAN resume "
            "text only — NO surrounding quotation marks, NO asterisks or markdown, NO "
            "parenthetical '(Note: ...)' notes, and NO suggestions or commentary "
            "addressed to the user. Just the finished bullet.\n"
...
def enhance_messages(data: dict, jd: str | None = None) -> list[dict]:
    ...
    return [
        {"role": "system", "content": (
            ...
            "Assisted, Helped, Responsible for, Worked on) — is concise, and KEEPS "
            "any metric already present but NEVER invents/estimates a number, "
            "percentage, amount or date and NEVER uses a placeholder like 'X%' or "
            "'$Y'. ALWAYS include a 'skills' "
            "section with 8-12 relevant skills drawn from the candidate's titles, "
            "summary and experience (real competencies they demonstrate; never "
            "fabricate). Each skill MUST be a SHORT 1-3 word keyword (e.g. 'Python', "
            "'Sales Strategy', 'Team Leadership') — NEVER a sentence, achievement or "
            "award title, and never put a metric in a skill. Output CLEAN resume text only — NO quotation marks "
            "around bullets, NO asterisks/markdown, NO '(Note: ...)' notes, NO "
            "commentary to the user. Keep all companies, titles, dates, education and "
            "facts EXACTLY as given — never invent employers, numbers, or experience "
            "that is not implied. Do not add or remove jobs. Keep the same top-level keys "
