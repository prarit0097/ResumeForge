...
def upload(request):
    ...
    data = structure.to_resume_json(raw_text)

    session_key = ensure_session_key(request)
    resume = services.create_resume(
        session_key, title="Enhanced resume", data=data,
    )
    response = redirect(reverse("builder:editor", args=[resume.id]) + f"?t={resume.edit_token}")
    response.set_cookie(
